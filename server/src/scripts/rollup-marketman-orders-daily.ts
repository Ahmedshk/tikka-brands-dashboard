/**
 * Idempotent: upserts `MarketManOrderDailyRollup` per location, buyerGuid, apiKind, and business day.
 * Order count buckets `businessDateAt` by calendar date in the location timezone (Mongo `$dateToString`).
 *
 * Local: npm run rollup-marketman-orders-daily -- --from 2026-03-01 --to 2026-03-07
 * Optional: `--locationId`. Omit `--from`/`--to` for yesterday per location TZ.
 */
import dotenv from "dotenv";
import path from "node:path";
import { fileURLToPath } from "node:url";
import mongoose from "mongoose";
import { connectDatabase } from "../config/database.js";
import { buildMarketManRollupForDay } from "../services/dailyRollupBuilder.service.js";
import type { MarketManOrderApiKind } from "../models/marketmanOrderCache.model.js";
import {
  iterBusinessDateKeysInclusive,
  parseRollupCliArgs,
  resolveRollupDateRangeForLocation,
} from "../utils/rollupScriptArgs.util.js";
import {
  distinctBuyerGuidsForMarketManRollup,
  loadLocationsForRollupScript,
} from "../utils/rollupLocations.util.js";
import { logger } from "../utils/logger.util.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
dotenv.config({ path: path.join(__dirname, "..", "..", ".env") });

const API_KINDS: MarketManOrderApiKind[] = ["sent", "delivery"];

async function main(): Promise<void> {
  const args = parseRollupCliArgs(process.argv.slice(2));
  try {
    await connectDatabase();
    const locations = await loadLocationsForRollupScript(args.locationId);
    if (locations.length === 0) {
      console.log("No locations matched; exiting.");
      await mongoose.disconnect();
      process.exit(0);
      return;
    }
    for (const loc of locations) {
      const buyerGuids = await distinctBuyerGuidsForMarketManRollup(
        String(loc._id),
        loc.marketManBuyerGuid,
      );
      if (buyerGuids.length === 0) {
        logger.info("MarketMan rollup: no buyerGuids for location", {
          locationId: String(loc._id),
        });
        continue;
      }
      const { fromKey, toKey } = resolveRollupDateRangeForLocation(
        args,
        loc.timezone,
      );
      const days = iterBusinessDateKeysInclusive(fromKey, toKey);
      for (const businessDateKey of days) {
        for (const buyerGuid of buyerGuids) {
          for (const apiKind of API_KINDS) {
            await buildMarketManRollupForDay(
              String(loc._id),
              buyerGuid,
              apiKind,
              businessDateKey,
              loc.timezone,
            );
            logger.info("MarketMan order rollup", {
              locationId: String(loc._id),
              buyerGuid,
              apiKind,
              businessDateKey,
            });
          }
        }
      }
    }
    console.log("\n✅ MarketMan order daily rollups complete.\n");
    await mongoose.disconnect();
    process.exit(0);
  } catch (error) {
    logger.error("rollup-marketman-orders-daily failed", error);
    console.error("\n❌ Rollup failed:", error);
    try {
      await mongoose.disconnect();
    } catch {
      /* ignore */
    }
    process.exit(1);
  }
}

void main();
