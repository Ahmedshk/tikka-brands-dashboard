...
function getReviewRatingValue(
  ...
  if (slice?.reviewRating != null) return String(slice.reviewRating);
  return '—';
}
...
function getRawNetSales(
  ...
  return getRawForPeriod(period, todaySlice?.netSalesToday, wtdSlice?.netSalesWeekToDate);
}
...
function getRawLaborCost(
  ...
  return getRawForPeriod(period, todaySlice?.laborCostToday, wtdSlice?.laborCostWeekToDate);
}
...
function buildNetSalesItem(
  ...
  return {
    ...
    rightIcon,
    loading: ctx.loading,
    ...(ctx.isDual && { period, onPeriodChange: setPeriod }),
  };
}
...
export function buildCommandCenterKPIItems(params: BuildCommandCenterKPIItemsParams): CommandCenterKPIItem[] {
  const {
    kpis,
    loading,
    canNetSales,
  ...
  const todaySlice: CommandCenterKPIsTodaySlice | undefined = isDual ? kpis.today : (kpis ?? undefined);
  const wtdSlice: CommandCenterKPIsWeekToDateSlice | undefined = isDual ? kpis.weekToDate : undefined;
  const ctx: SliceContext = { todaySlice, wtdSlice, isDual, loading };

  const items: CommandCenterKPIItem[] = [];
  if (canNetSales) {
    items.push(buildNetSalesItem(ctx, netSalesPeriod, setNetSalesPeriod, icons.dollar));
  }
  if (canLaborCost) {
    items.push(buildLaborCostItem(ctx, laborCostPeriod, setLaborCostPeriod, icons.laborCost));
  }
  if (canReviewRating) {
    items.push(
